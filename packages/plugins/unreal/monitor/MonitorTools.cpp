...
void UMCPMonitorTools::RegisterTools()
{
    FMCPProtocolHandler::Get().RegisterTool("getCameraView", FMCPToolDelegate::CreateUObject(this, &UMCPMonitorTools::getCameraView));
}

TSharedPtr<FJsonObject> UMCPMonitorTools::getCameraView(const TSharedPtr<FJsonObject>& Params)
{
    UE_LOG(LogMCPPlugin, Display, TEXT("Executing getCameraView in Unreal Engine"));
    ...
    try
    {
        ...
        // Extract complex parameter perspective 
        const TSharedPtr<FJsonValue>* perspectiveValue = nullptr;
        if (Params->TryGetField("perspective", perspectiveValue)) {
        }
        
        // TODO: Implement actual Unreal Engine API calls for getCameraView
        // This is a placeholder implementation
    ...
    {
        UE_LOG(LogMCPPlugin, Error, TEXT("Error in getCameraView: %s"), *FString(Exception.what()));
        Response->SetBoolField("success", false);
        Response->SetStringField("error", Exception.what());
    }
