...
type CombinedCrudOperations<T extends Record<string, z.ZodTypeAny>> = {
...
function createCrudOperations<T extends EntitySchemas>(entities: Partial<T>) {
  const operations = {} as CombinedCrudOperations<T>;

  Object.entries(entities).forEach(([entityName, entitySchema]) => {
    const pascalCase = (entityName.charAt(0).toUpperCase() +
      entityName.slice(1)) as keyof T;
    ...
    operations[`create${pascalCase}s`] = {
      ...
      returns: OperationResponse.extend({
        ids: z.array(z.string()).describe(`IDs of the created ${pluralName}`),
      }),
    };
    ...
    operations[`get${pascalCase}s`] = {
      ...
      parameters: z.object({
        ids: z.array(z.string()).describe(`${pascalCase} identifiers`),
      }),
      returns: OperationResponse.extend({
        items: z.array(entitySchema).describe(`Array of ${pluralName} objects`),
      }),
    };
          ...
          .array(
            z
              .object({
                ids: z
                  .array(z.string())
                  .describe(`${pascalCase} identifiers to update`),
              })
              .merge(entitySchema.partial().omit({ id: true }))
          )
          .describe(`Array of ${pluralName} to update with their IDs`),
      }),
